...
import { NPCStrategy, Player } from './base/NPCStrategy.js';
...
export class NPCManager {
    private availableStrategies: Map<string, () => NPCStrategy>;
    private currentStrategy: NPCStrategy;
     ...
     */
    getAvailableStrategies(): Array<{key: string, name: string, difficulty: string, description: string}> {
        const strategies = [];
     ...
     * @returns 現在のストラテジー
     */
    getCurrentStrategy(): NPCStrategy {
     ...
     * @returns ストラテジー情報
     */
    getCurrentStrategyInfo(): {name: string, difficulty: string, description: string} {
        return {
